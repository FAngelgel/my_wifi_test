...
{
    // 1. Safety check: Ensure drivers are ready
    ...
    // 2. If length is 0, there is nothing to do
    if (len_bytes == 0) return true;

    // 3. Protect the bus with the Mutex
    if (xSemaphoreTake(g_spi_mutex, pdMS_TO_TICKS(100)) == pdTRUE)
    {
        spi_transaction_t t = {};
        
        // Convert bytes to bits for the ESP-IDF driver
        t.length   = len_bytes * 8; 
        
        // If rx is NULL, set rxlength to 0 (Send Only mode)
        // If rx is NOT NULL, set it to the requested length
        t.rxlength = (rx == nullptr) ? 0 : len_bytes * 8;

        t.tx_buffer = tx; // If tx is NULL, ESP32 sends "dummy" zeros
        t.rx_buffer = rx; // If rx is NULL, ESP32 ignores incoming data

        // 4. Perform the physical hardware transfer
        esp_err_t ret = spi_device_transmit(g_dev, &t);

        // 5. CRITICAL: Always release the Mutex
    ...
    {
        ESP_LOGE(TAG, "SPI Mutex Timeout - Bus Busy");
        return false;
    }
}
