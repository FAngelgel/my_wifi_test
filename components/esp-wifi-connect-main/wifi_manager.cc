...
void WifiManager::StartStation() {
    ...
    // Auto-stop config AP if active (unless configured to keep AP during STA)
...
void WifiManager::StartConfigAp() {
    ...
    config_ap_->OnExitRequested([this]() {
        ...
        if (!config_.keep_ap_on_station_start) {
            StopConfigAp();
        }
        // Start station to connect using saved credentials.
        StartStation();
    });
